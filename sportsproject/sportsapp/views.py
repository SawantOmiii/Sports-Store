...
def Signup(request):
    if request.user.is_authenticated:
        ...
        if request.method == 'POST':
            ...
            subject  = f'Welcome To Sports24 {username}'
            message = f"""
                Dear {username},
                    Your have Successfully register To Sports24
                    with {email} this email,

                    Thank You For selecting the Sports24 store.

                    We wish to buy some sports product and stay in Touch with us.
...
def address(request,id):
    if request.user.is_authenticated:
        ...
        if id not in list(order_id):
            ...
            data_detail = Product.objects.filter(pk=id)     
            print(data_detail)     
            return render(request,'user/address.html',{'adata':data,'product_id':product_id,'data':data_detail})
        else:
...
def Payment(request,aid,pid):
    if request.user.is_authenticated:
        client = razorpay.Client(auth=("rzp_test_4ytOhGKWH1NbKZ", "Sfo9acBcIRqQLZQQMAW3J3Vs"))
